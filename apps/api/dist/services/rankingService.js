...
exports.freshnessScore = freshnessScore;
exports.computeRankingScore = computeRankingScore;
exports.recalculateAllRankings = recalculateAllRankings;
exports.startRankingCron = startRankingCron;
const db_1 = require("../config/db");
function normalizeRating(avg) {
    return Math.max(0, (avg - 1) / 4);
}
function usageScore(hireCount, globalMax) {
...
function computeRankingScore(agent, globalMaxHires) {
...
async function recalculateAllRankings() {
    const { rows: [maxRow] } = await db_1.pool.query(`SELECT MAX(hire_count) as max_hires FROM agents WHERE status = 'published'`);
    const globalMax = maxRow?.max_hires || 1;
    const { rows: agents } = await db_1.pool.query(`SELECT id, rating_avg, hire_count, quality_score, published_at FROM agents WHERE status = 'published'`);
    for (const agent of agents) {
        const score = computeRankingScore(agent, globalMax);
        await db_1.pool.query(`UPDATE agents SET ranking_score = $1, updated_at = NOW() WHERE id = $2`, [score, agent.id]);
    }
    // Save daily snapshot
