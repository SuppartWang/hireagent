...
async function agentRoutes(app) {
    ...
    app.get('/', async (req, reply) => {
        const { page = 1, limit = 20, sort = 'ranking', category, capabilities, language, model, q, } = req.query;
    ...
    `;
        const [countResult, dataResult] = await Promise.all([
            ...
            db_1.pool.query(dataQuery, [...params, Number(limit), offset]),
        ]);
        return reply.send({
    ...
    app.post('/', { preHandler: [app.authenticate] }, async (req, reply) => {
        const user = req.user;
        const body = req.body;
        const { nameZh, nameEn, descriptionZh, descriptionEn, taglineZh, taglineEn, systemPrompt, systemPromptLang, mcpConfig, category, tags, capabilities, supportedModels, languageSupport, avatarUrl, coverUrl, demoVideoUrl, } = body;
        if (!nameZh || !descriptionZh || !systemPrompt) {
            return reply.code(400).send({ error: '缺少必填字段：名称、描述、系统提示词' });
        }
        const slug = (0, slugify_1.default)(nameZh) + '-' + Date.now().toString(36);
        const qualityScore = (0, pointsEngine_1.computeQualityScore)({
    ...
    app.put('/:id', { preHandler: [app.authenticate] }, async (req, reply) => {
        ...
        const user = req.user;
        const body = req.body;
        const { rows: existing } = await db_1.pool.query(`SELECT creator_id, version FROM agents WHERE id = $1`, [id]);
        if (!existing[0])
            return reply.code(404).send({ error: '智能体不存在' });
        if (existing[0].creator_id !== user.id && !user.isAdmin)
            return reply.code(403).send({ error: '无权限修改此智能体' });
        // Save version history
        const { rows: current } = await db_1.pool.query(`SELECT system_prompt, mcp_config, version FROM agents WHERE id = $1`, [id]);
        await db_1.pool.query(`INSERT INTO agent_versions (agent_id, version, system_prompt, mcp_config) VALUES ($1,$2,$3,$4)`, [id, current[0].version, current[0].system_prompt, current[0].mcp_config]);
        const qualityScore = (0, pointsEngine_1.computeQualityScore)({
    ...
    // Publish agent
    app.post('/:id/publish', { preHandler: [app.authenticate] }, async (req, reply) => {
        const { id } = req.params;
        const user = req.user;
        const { rows: existing } = await db_1.pool.query(`SELECT creator_id, status FROM agents WHERE id = $1`, [id]);
        if (!existing[0])
            return reply.code(404).send({ error: '智能体不存在' });
        if (existing[0].creator_id !== user.id)
            return reply.code(403).send({ error: '无权限' });
        const isFirst = existing[0].status === 'draft';
        const { rows } = await db_1.pool.query(`UPDATE agents SET status='published', published_at=COALESCE(published_at, NOW()), updated_at=NOW()
       WHERE id=$1 RETURNING *`, [id]);
        if (isFirst) {
            // Check if user's first agent publish for bonus points
            const { rows: prevAgents } = await db_1.pool.query(`SELECT COUNT(*) FROM agents WHERE creator_id=$1 AND status='published' AND id != $2`, [user.id, id]);
            if (Number(prevAgents[0].count) === 0) {
                await (0, pointsEngine_1.awardPoints)(user.id, id, shared_1.POINT_RULES.FIRST_PUBLISH, 'FIRST_PUBLISH', '首次发布智能体奖励', 'First agent published bonus');
            }
        }
    ...
    app.post('/:id/hire', async (req, reply) => {
        const { id } = req.params;
        const { hireType } = req.body;
        const user = req.user;
        const { rows } = await db_1.pool.query(`UPDATE agents SET hire_count = hire_count + 1, updated_at = NOW()
       WHERE id = $1 AND status = 'published' RETURNING creator_id`, [id]);
        if (!rows[0])
            return reply.code(404).send({ error: '智能体不存在' });
        await db_1.pool.query(`INSERT INTO agent_hires (agent_id, user_id, hire_type) VALUES ($1, $2, $3)`, [id, user?.id || null, hireType || 'unknown']);
        // Award points to creator
        if (rows[0].creator_id && rows[0].creator_id !== user?.id) {
            await (0, pointsEngine_1.awardPoints)(rows[0].creator_id, id, shared_1.POINT_RULES.AGENT_HIRED, 'AGENT_HIRED', '智能体被使用', 'Agent was hired/exported');
        }
        return reply.send({ success: true });
    });
    // Toggle bookmark
    app.post('/:id/bookmark', { preHandler: [app.authenticate] }, async (req, reply) => {
        const { id } = req.params;
        const user = req.user;
        const { rows: existing } = await db_1.pool.query(`SELECT 1 FROM agent_bookmarks WHERE user_id=$1 AND agent_id=$2`, [user.id, id]);
        if (existing.length > 0) {
            await db_1.pool.query(`DELETE FROM agent_bookmarks WHERE user_id=$1 AND agent_id=$2`, [user.id, id]);
            return reply.send({ bookmarked: false });
        }
        else {
            await db_1.pool.query(`INSERT INTO agent_bookmarks (user_id, agent_id) VALUES ($1, $2)`, [user.id, id]);
